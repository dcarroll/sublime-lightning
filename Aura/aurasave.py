...
class AuraNewAppCommand(sublime_plugin.WindowCommand):
  def run(self, dirs):
    self.dirs = dirs
  ...
  def on_done(self, file_name):
    app = aurasave.Helper(self.window).make_bundle_file(file_name, "app","<aura:application>\n\n</aura:application>", self.dirs)
    return

  def is_visible(self, dirs):
    return aurasave.Helper(self.window).bundle_op_is_visible(dirs)

class AuraNewComponentCommand(sublime_plugin.WindowCommand):
  def run(self, dirs):
    self.dirs = dirs
    self.window.show_input_panel("Component Name:", "", self.on_done, None, None)
    pass

  def on_done(self, file_name):
    app = aurasave.Helper(self.window).make_bundle_file(file_name, "cmp",
      "<aura:component>\n\n</aura:component>", self.dirs)
    return

  def is_visible(self, dirs):
    return aurasave.Helper(self.window).bundle_op_is_visible(dirs)

class AuraNewEventCommand(sublime_plugin.WindowCommand):
  def run(self, dirs):
    self.dirs = dirs
    self.window.show_input_panel("Event Name:", "", self.on_done, None, None)
    pass

  def on_done(self, file_name):
    app = aurasave.Helper(self.window).make_bundle_file(file_name, "evt",
      '<aura:event type="APPLICATION">\n\n</aura:event>', self.dirs)
    return

  def is_visible(self, dirs):
    return aurasave.Helper(self.window).bundle_op_is_visible(dirs)

class AuraNewControllerCommand(sublime_plugin.WindowCommand):
  def run(self, dirs):
    self.dirs = dirs
  ...
  def is_visible(self, dirs):
    helper = aurasave.Helper(self.window)
    if len(dirs) == 0:
      return False
    hasFile = helper.has_this_file(dirs[0], \
      os.path.basename(dirs[0]) + "Controller.js")
    isValidBundle = helper.is_bundle_type(dirs, "app") or \
      helper.is_bundle_type(dirs, "cmp")
    return aurasave.Helper(self.window).file_op_is_visible(dirs) and \
      isValidBundle and not hasFile

class AuraNewHelperCommand(sublime_plugin.WindowCommand):
  def run(self, dirs):
    self.dirs = dirs
...
class AuraNewModelCommand(sublime_plugin.WindowCommand):
  def run(self, dirs):
    self.dirs = dirs
...
class AuraNewStyleCommand(sublime_plugin.WindowCommand):
  def run(self, dirs):
    self.dirs = dirs
...
class AuraDeleteCommand(sublime_plugin.WindowCommand):
  def run(self, files):
    #print(files)
    for f in files:
      print(f)
...
class AuraDeleteBundleCommand(sublime_plugin.WindowCommand):
  def run(self, dirs):
    for d in dirs:
      command = 'delete -f=' + d
      self.window.run_command('exec',{ 'cmd':["force", "aura", command] })
